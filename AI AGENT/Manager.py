...
class ManagerAgent:
    def __init__(self, df: pd.DataFrame):
        ...
        prediction_prompt = (
            ...
            "[Visual: table OR chart — see rules below]\n\n"
            "**Key takeaway:** [1–2 sentences of plain-English insight. Nothing more.]\n\n"
            "── VISUAL RULES ─────────────────────────────────────────────────────────\n"
            "• Rankings / lists (at-risk customers, top products, CLV) → markdown table.\n"
            "  Table columns must be bold headers. Include units (£, %, days) in headers.\n"
            "• Trends / forecasts / time-series → line chart via execute_python.\n"
            "  Use seaborn lineplot. Mark historical data in blue, forecasted in orange dashed.\n"
            "  Always annotate the last actual point and each forecast point with its value.\n"
            "• Comparisons (growth vs decline, before vs after) → horizontal bar chart.\n"
            "  Color positive bars green (#2ecc71), negative bars red (#e74c3c).\n"
            "• Mixed / summary → table first, then chart.\n"
            "NEVER produce a response with only plain text. If a chart is generated it will "
            "be shown automatically — do NOT include base64 or file paths in the text.\n\n"
            "── STYLE RULES ──────────────────────────────────────────────────────────\n"
            ...
            "• Forecasts: always add a one-line disclaimer that results are pattern-based.\n"
            "• Respond in the same language the user wrote in (Hebrew or English).\n\n"
            "── DATA RULES ───────────────────────────────────────────────────────────\n"
            "1. Call a tool before answering — never guess.\n"
            "2. If the user mentions a product by name, call search_products first.\n"
            "3. If execute_python fails, diagnose and retry up to 3 times.\n"
        )
